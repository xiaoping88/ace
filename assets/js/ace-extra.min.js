...
ace.vars.webkit = !!ua.match(/AppleWebKit/i);
ace.vars.safari = !!ua.match(/Safari/i) && !ua.match(/Chrome/i);
ace.vars.android = ace.vars.safari && !!ua.match(/Android/i);
ace.vars.ios_safari = !!ua.match(/OS ([4-9])(_\d)+ like Mac OS X/i) && !ua.match(/CriOS/i);
...
ace.vars.non_auto_fixed && jQuery("body").addClass("mob-safari");
ace.vars.transition = !!jQuery.support.transition;
...
ace.helper.unCamelCase = function(a) {
	return a.replace(/([a-z])([A-Z])/g, function(a, b, c) {
		return b + "-" + c.toLowerCase()
	})
};

ace.helper.strToVal = function(a) {
...
ace.helper.getAttrSettings = function(b, c, d) {
	...
	for (var g in c)
		if (c.hasOwnProperty(g)) {
			var h, i = 1 == e ? c[g] : g,
				j = ace.helper.unCamelCase(i.replace(/[^A-Za-z0-9]{1,}/g, "-")).toLowerCase();
			if (!(h = b.getAttribute(d + j))) continue;
			f[i] = ace.helper.strToVal(h)
		}
	return f
};
...
ace.helper.getComputedStyle = "getComputedStyle" in window ? function(a, b) {
		return a.offsetHeight, window.getComputedStyle(a).position == b
} : function(b, c) {
	return b.offsetHeight, $(b).css("position") == c
};